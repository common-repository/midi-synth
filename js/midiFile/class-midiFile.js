/**
 * Description: Read (and soon edit) a MIDI file in a given ArrayBuffer
 *
 * @package     midi-Synth
 * @category    class js
...
class MIDIFile {
  ...
  constructor (buffer, strictMode) {
    this.MIDIEvents = new MIDIEvents();
    var track;
    var curIndex;
    var i;
    var j;
    ...
    if (!buffer) {
      ...
      buffer = this.ensureArrayBuffer(buffer);
  ...
  getEvents(type, subtype) {
    ...
    if (1 !== format || 1 === this.tracks.length) {
      for (i = 0, j = this.tracks.length; i < j; i++) {
        ...
        events = this.MIDIEvents.createParser(
      ...
      for (i = 0, j = this.tracks.length; i < j; i++) {
        trackParsers[i] = {};
        trackParsers[i].parser = this.MIDIEvents.createParser(
          this.tracks[i].getTrackContent(),
          0,
          false
        );
  ...
  getTrackEvents(index) {
    ...
    parser = this.MIDIEvents.createParser(
  ...
  setTrackEvents(index, events) {
    ...
    bufferLength = this.MIDIEvents.getRequiredBufferLength(events);
    destination = new Uint8Array(bufferLength);
    this.MIDIEvents.writeToTrack(events, destination);
