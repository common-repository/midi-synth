...
function WebAudioTinySynthCore(target) {
  Object.assign(target,{
    ...
    /*@@guiEND*/
    arrNotes:["C-1","C#-1","D-1","D#-1","E-1","F-1","F#-1","G-1","G#-1","A-1","A#-1","B-1",
...
*/
          ...
          if(this.playing && this.song.ev.length>0){
            ...
            while(this.actx.currentTime+this.preroll>this.playTime){
              if(e.m[0]==0xff51){
                ...
                let tempoBPM = document.getElementById("tempoBPM");
                tempoBPM.value=this.song.tempo;
                tempoBPM.style.background="#C3C3C3";
                setTimeout( () => {
                  tempoBPM.style.background="white";
                }, 250);
              }else{
                let ch = e.m[0]&0xf;
    ...
    loadMIDI:(data)=>{
      ...
      function Msg(song,tick,s,i,tracksName,numtrk,tracksInfo,timeSignature){
        var v=s[i];
        datalen=1;
        if((v&0x80)==0)
          v=runst,datalen=0;
        ...
        switch(v&0xf0){
        case 0xc0: case 0xd0:
          let obj = {numtrk:numtrk, ch:v&0xf0>>4, prog:s[i+datalen]};
          let found = false;
          tracksInfo.forEach(function(item, index){
            if(numtrk == item.numtrk) found = true;
          });
          if(false == found) tracksInfo.push(obj);
...
*/
          ...
          case 0xff:
            ...
            case 0x02:
              ...
              break;
            case 0x03:
              tracksName.push({numtrk:numtrk, nametrk:GetStr(s, i + datastart, datalen - datastart)});
              break;
            case 0x2f:
              return 1;
            case 0x51:
      ...
      this.formatMidi = fmt;
      this.tracksCount = numtrk;
      this.ticksPerBeat = Get2(s, 12);
      ...
      for(let tr=0;tr<numtrk;++tr){
        ...
        tracksContent.push(new DataView(s.buffer, idx, len+8));
        ...
        if(hd.toString()=="77,84,114,107") {//MTrk
          ...
          for(;;) {
            ...
            j += datalen;
            var e = Msg(this.song, tick, s, idx + 8 + j, tracksName, tr, tracksInfo, timeSignature);
            j += datalen;
            if(e)
              break;
          }
      ...
      tracksName.forEach(function(trkName, index){
        let found = false;
        tracksInfo.forEach(function(item, index){
          if(trkName.numtrk == item.numtrk) found = true;
          if(!item.nametrk && item.numtrk == trkName.numtrk) item.nametrk = trkName.nametrk;
        });
        if(false == found) tracksInfo.push(trkName);
      });
      tracksInfo.sort( (p1, p2) => {
        if (p1.numtrk < p2.numtrk) return -1;
        if (p1.numtrk > p2.numtrk) return 1;
        return 0;
      });
    ...
    getTracksCount(){
      return this.tracksCount-1;
    },
    getTracksContent(nunTrack){
      if(nunTrack){
        return this.tracksContent[nunTrack];
      }else{
        return this.tracksContent;
      }
    },
    getFormatMidi(){
      return this.formatMidi;
    },
    getTicksPerBeat(){
    ...
    _note:(t,ch,n,v,p)=>{
      ...
      if (0 != this.convDuration) {
        var length   = this.actx.sampleRate * this.convDuration;
        var impulse  = this.audioContext.createBuffer(2, length, this.actx.sampleRate);
    ...
    send:(msg,t)=>{    /* send midi message */
      const ch=msg[0]&0xf;
      const cmd=msg[0]&~0xf;
      const speaker   = document.getElementById("speaker_"+Number(ch+1));
      const speakerOn = document.getElementById("speakerOn_"+Number(ch+1));
      const speakerOff = document.getElementById("speakerOff_"+Number(ch+1));
      ...
      case 0x90:
        ...
        if(speakerOn && speakerOff && "none" == speakerOff.style.display)
          speakerOn.setAttribute("style","display:block;width:16px;height:16px;cursor:pointer;");
        if(speaker && speakerOff && "none" == speakerOff.style.display)
